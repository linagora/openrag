...
The `extra` field in API responses is a JSON string: `{"sources": [filtered_source_list], "all_retrieved_sources": [all_retrieved_sources]}`. `all_retrieved_sources` always carries the complete retrieval set — captured before the context-token-budget truncation in `format_context()`/`format_web_context()`, so it also includes documents/web results that didn't fit in the prompt — unfiltered by citation, useful for debugging and RAG evaluation regardless of what the model cited.
