...
def _partition_update_sql(conn: _FakeConn) -> list[str]:
    return [sql for sql, _ in conn.updates if "SET embedder" in sql]
...
def test_indexed_partitions_on_the_alias_are_pinned_to_the_single_default(monkeypatch, migration) -> None:
    ...
    # Empty partitions keep the alias and follow the next set-default, as the
    # application's own pinning leaves them (#939).
    [update] = _partition_update_sql(conn)
    assert _HOLDS_FILES in update
...
def test_nothing_to_pin_without_exactly_one_default(monkeypatch, migration, defaults) -> None:
    """No indexed partition rides the alias, so which embedder it means never
    comes up — empty partitions on it have no rows to route."""
    ...
    [count] = conn.counts
    assert _HOLDS_FILES in count


@pytest.mark.parametrize("defaults", [[], ["a", "b"]])
def test_indexed_partitions_on_the_alias_stop_the_migration_without_one_default(
    monkeypatch, migration, defaults
) -> None:
    ...
    with pytest.raises(RuntimeError, match="2 partition\\(s\\) holding files use the 'default' embedder alias"):
        migration.upgrade()
