...
class TestDistributedSemaphoreReleaseStarvation:
    ...
    back, the waiters never get one, and every caller of the semaphore is
    wedged until the actor is restarted. Callers that fan out one acquire
    per item up-front (one per image, one per chunk) reach the cap easily.
    """
