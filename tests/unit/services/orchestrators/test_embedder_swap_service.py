...
def _make(files=None, chunks=None, *, repo=None, task_count=0, monkeypatch=None, stored_endpoints=None):
    files = (
        files
...
async def test_a_resumed_swap_counts_on_from_the_files_it_already_did(monkeypatch):
    ...
    async def record(partition, **fields):
        progress.append(fields)
        return await original_update(partition, **fields)
    ...
    assert parts["embedder"].calls == [["hello"]]
    assert progress == [{"files_total": 2, "files_done": 1}, {"files_done": 2}]
    assert repo.swap["status"] == "completed"
