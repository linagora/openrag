...
@pytest.mark.asyncio
async def test_snapshot_consumes_the_resolved_plan_configuration_and_fingerprint():
    class PlanRetrieval(_Retrieval):
        def __init__(self):
            super().__init__()
            self.plan_calls = []
        ...
        async def resolve_retrieval_plan(self, partitions, **kwargs):
            self.plan_calls.append((partitions, kwargs))
            public = await super().resolved_public_retrieval_configuration(partitions)
            self.resolved_plan = SimpleNamespace(
                public_configuration=public,
                configuration_fingerprint=canonical_fingerprint(public),
            )
            return self.resolved_plan

        async def resolved_public_retrieval_configuration(self, _partitions):
            raise AssertionError("snapshot must consume the resolved plan directly")
