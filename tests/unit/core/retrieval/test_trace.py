...
def test_candidates_from_chunks_keeps_only_identifiers_ranks_and_public_scores():
    ...
    scored = ScoredChunk.from_chunk(
        Chunk(id="scored", document_id="doc-2", text="another private body", partition="science"),
        vector_score=0.9,
        rerank_score=0.8,
        combined_score=0.7,
    )
...
def test_candidates_from_chunks_honors_the_collection_capacity_before_projection():
    chunks = [Chunk(id=f"chunk-{index}", text="private") for index in range(5)]

    candidates = candidates_from_chunks(chunks, limit=2)

    assert [candidate.id for candidate in candidates] == ["chunk-0", "chunk-1"]


def test_safe_serialization_enforces_a_global_byte_budget():
    oversized = {
        "original_query": "question " + "q" * 100_000,
        "query_traces": [
            {
                "query": f"query-{index}-" + "é" * 3_000,
                "partition": f"partition-{index}",
                "stages": [
                    {
                        "name": "dense_after_threshold",
                        "status": "complete",
    ...
    trace = safe_public_value(oversized)

    assert len(json.dumps(trace).encode()) <= MAX_TRACE_SERIALIZED_BYTES
    assert trace["trace_truncated"] is True
    assert trace["serialized_size_limit"] == MAX_TRACE_SERIALIZED_BYTES
...
def test_trace_applies_one_candidate_budget_across_nested_stages():
    parent = RetrievalTraceBuilder("request", "question")
    produced_candidates = 0
    for query_index in range(60):
        child = RetrievalTraceBuilder(
            f"child-{query_index}",
            f"query-{query_index}",
        ...
        child.record_stage("dense_after_threshold", status="complete", candidates=candidates)
        parent.record_query_trace(child)

    trace = parent.finish(configuration_fingerprint="fingerprint")
    serialized_candidates = sum(
        len(stage["candidates"]) for query_trace in trace["query_traces"] for stage in query_trace["stages"]
    )

    assert serialized_candidates < produced_candidates
    assert serialized_candidates == trace["candidate_limit"]
    assert trace["candidates_truncated"] is True


def test_primary_query_traces_have_priority_over_comparison_traces():
    parent = RetrievalTraceBuilder("request", "question")
    primary = RetrievalTraceBuilder("primary", "primary query")
    parent.record_query_trace(primary)
    parent.comparisons["original_query"] = {
        "status": "complete",
        "query_traces": [{"query": f"comparison-{index}"} for index in range(500)],
    }

    trace = parent.finish(configuration_fingerprint="fingerprint")

    assert trace["query_traces"][0]["query"] == "primary query"
    assert len(trace["comparisons"]["original_query"]["query_traces"]) == 499
    assert trace["query_traces_truncated"] is True


def test_shared_diagnostics_budget_caps_candidates_while_preserving_later_stage_metadata():
    diagnostics = RetrievalDiagnosticsContext(candidate_limit=3)
    parent = RetrievalTraceBuilder("request", "question", diagnostics=diagnostics)
    first = RetrievalTraceBuilder("first", "first query", diagnostics=diagnostics)
    second = RetrievalTraceBuilder("second", "second query", diagnostics=diagnostics)

    first.record_stage(
        "dense_after_threshold",
        status="complete",
        candidates=[TraceCandidate(id=f"first-{index}", rank=index + 1) for index in range(3)],
    )
    second.record_stage(
        "dense_after_threshold",
        status="complete",
        candidates=[TraceCandidate(id=f"second-{index}", rank=index + 1) for index in range(2)],
    )
    second.record_stage("final", status="complete", candidates=[])
    parent.record_query_trace(first)
    parent.record_query_trace(second)

    assert sum(len(stage.candidates) for child in parent.query_traces for stage in child.stages) == 3
    second_dense = next(stage for stage in parent.query_traces[1].stages if stage.name == "dense_after_threshold")
    second_final = next(stage for stage in parent.query_traces[1].stages if stage.name == "final")
    assert second_dense.candidate_count == 2
    assert second_dense.candidates == []
    assert second_final.status == "complete"
...
def test_candidate_budget_stops_serializing_after_the_limit(monkeypatch):
    model_dump_calls = 0
    original_model_dump = TraceCandidate.model_dump

    def counted_model_dump(candidate, *args, **kwargs):
        nonlocal model_dump_calls
        model_dump_calls += 1
        return original_model_dump(candidate, *args, **kwargs)

    monkeypatch.setattr(TraceCandidate, "model_dump", counted_model_dump)
    parent = RetrievalTraceBuilder("request", "question")
