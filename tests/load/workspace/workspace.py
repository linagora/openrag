...
Usage:
  cd tests/load/workspace && docker compose up -d
...
def setup_postgres(engine):
    ...
    with engine.begin() as conn:
        ...
        conn.execute(text("CREATE INDEX ix_partition_file ON files(partition_name, file_id)"))
        conn.execute(
            text("""
...
def insert_data(
    ...
    with engine.begin() as conn:
        ...
        for fid in all_file_ids:
            conn.execute(
                text("INSERT INTO files (file_id, partition_name) VALUES (:fid, :p)"),
        ...
        for ws_id, file_ids in assignments.items():
            for fid in file_ids:
                conn.execute(
                    text("INSERT INTO file_workspaces (file_id, workspace_id, partition_name) VALUES (:fid, :ws, :p)"),
...
def search_approach_a(
    ...
    if search_type == "dense":
        ...
        t, _ = timed(lambda: _hybrid_search(col, query_vector, query_text, filter_expr))
...
def pg_resolve_workspace(engine, workspace_id: str) -> tuple[float, list[str]]:
    ...
    def _query():
        with engine.connect() as conn:
            rows = conn.execute(
                text("SELECT file_id FROM file_workspaces WHERE workspace_id = :ws AND partition_name = :p"),
...
def bench_write_approach_a_partial(client: MilvusClient) -> list[float]:
    ...
    for _ in range(MEASURED_RUNS):
        ...
        if cleanup:
            client.upsert(collection_name=COLLECTION, data=cleanup, partial_update=True)
...
def bench_write_approach_b(engine) -> list[float]:
    ...
    for _ in range(MEASURED_RUNS):
        ...
        with engine.begin() as conn:
            conn.execute(
                text("DELETE FROM file_workspaces WHERE file_id = :fid AND workspace_id = :ws"),
...
def main(output_file: str | None = None):
    ...
    if client.has_collection(COLLECTION):
        ...
        if count == expected_entities:
            # Also check PG
            with engine.connect() as conn:
                pg_count = conn.execute(text("SELECT count(*) FROM file_workspaces")).scalar()
        ...
        for search_type in ["dense", "hybrid"]:
            ...
            for run in range(MEASURED_RUNS):
                ...
                if len(file_ids) <= effective_batch:
                    t_search = search_approach_b_single(client, col, file_ids, qv, qt, search_type)
                    t_merge = 0.0
                else:
            ...
            batches = max(1, (num_files + effective_batch - 1) // effective_batch) if num_files > effective_batch else 0
    ...
    summary_lines.append(
        f"\n  Write: A(full)={wa_full['median']:.2f}ms  A(partial)={wa_partial['median']:.2f}ms  B={wb['median']:.2f}ms"
    )
    ...
    if output_file:
        with open(output_file, "w") as f:
            ...
            f.write("## Approaches\n\n")
            f.write(
                "- **Approach A — Milvus ARRAY_CONTAINS**: Each chunk stores a "
            ...
            f.write(
                "- **Approach B — PostgreSQL resolution + file_id IN**: Workspace "
            ...
            f.write("## Setup\n\n")
            f.write(
                f"- **Data**: {FILES:,} files x {CHUNKS_PER_FILE} chunks = "
