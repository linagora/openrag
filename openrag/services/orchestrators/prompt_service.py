...
_PROMPT_FORMAT_FIELDS: dict[str, frozenset[str]] = {
    # ``custom_prompt`` is optional (see prepend_system_prompt): a client-pinned
    # leading system message, wrapped as untrusted content. Must stay in this
    # allow-list or the bundled disk templates (which contain it) fail
    # seed-time validation and the library default for these types is silently
    # never created.
