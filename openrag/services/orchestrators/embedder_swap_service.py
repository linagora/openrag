"""EmbedderSwapService — move a partition to another embedder in place.
...
recorded with another model or field — the repair for the drift warning reports
...
runner per swap across processes. The target endpoint is read from the database
before embedding, since the process that runs the swap need not be the one it
was started on, nor the one that last edited the endpoint.
"""
...
class EmbedderSwapService:
    ...
    def __init__(
        ...
        *,
        ...
        collection: str,
        model_endpoint_repo: ModelEndpointRepository | None = None,
        refresh_endpoints: Callable[[], Awaitable[None]] | None = None,
        ...
        self._model_endpoint_repo = model_endpoint_repo
        self._refresh_endpoints = refresh_endpoints
    ...
    async def start(self, partition: str, target_embedder: str) -> dict:
        ...
        embedder, field = await self._resolve_target(target_embedder)
        ...
        async with self._partition_service.operation_lock(partition):
            ...
            # A copy runs outside the fence, holding the copy lock until its file row exists.
            ...
            if copy_in_progress is not None and await copy_in_progress(partition):
                raise ConflictError(
                    f"A file is being copied into partition '{partition}'. "
                    "Start the embedder swap once the copy finishes, so it is re-embedded too.",
                    code="INDEXING_IN_PROGRESS",
                )
            row = await self._partition_repo.get_partition_row(partition)
            if row is None:
                raise PartitionNotFoundError(f"Partition '{partition}' does not exist.")
            files = await self._files_to_reembed(partition, embedder, field)
    ...
    async def _reembed_partition(self, swap: dict) -> None:
        ...
        # Resolved again here, not carried from the start: this is the process
        # that embeds, and it may not be the one the swap was started on.
        embedder, field = await self._resolve_target(target)

        # Listed again rather than trusted from the start: files deleted since
        # need no work. On a resume, the files an earlier run re-embedded are
        # recorded with the target already: they drop out here and stay
        # counted in files_done.
        files = await self._files_to_reembed(partition, embedder, field)
        done = swap.get("files_done") or 0
        await self._progress(partition, files_total=done + len(files), files_done=done)
        for file in files:
    ...
    async def _files_to_reembed(self, partition: str, embedder: Embedder, field: str) -> list[dict]:
        ...
        return [
            file
            for file in await self._document_repo.list_file_embedders(partition)
            if model_name is None
            or file.get("embedder_model_name") != model_name
            or file.get("embedder_vector_field") != field
        ]
    ...
    async def _resolve_target(self, target_embedder: str) -> tuple[Embedder, str]:
        """The target's embedder and field, built from the endpoint as the database has it."""
        await self._refresh_if_edited(target_embedder)
        field = self._target_vector_field(target_embedder)
        return self._embedder_factory(target_embedder), field

    async def _refresh_if_edited(self, target_embedder: str) -> None:
        """Reload this process's endpoint registry if the target was edited elsewhere.

        The registry is a cache, loaded at startup and on this process's own
        endpoint writes. With several API processes, an edit committed by
        another one leaves it stale, and a swap embedding with the endpoint as
        it was fills the target field with vectors no query can match, under
        provenance naming the old model — invisible, since the field is the one
        the partition is about to read. The stored fingerprint decides, the
        same one the indexer checks before it records a file. Edits to a
        running swap's target are refused, so what this reads holds until the
        swap ends.
        """
        repo, refresh = self._model_endpoint_repo, self._refresh_endpoints
        if repo is None or refresh is None or target_embedder == DEFAULT_ENDPOINT_ALIAS:
            return
        row = await repo.get(target_embedder, "embedder")
        if row is None:
            return  # deleted, or never there: _target_vector_field answers for it
        stored = embedder_fingerprint(row.endpoint, row.model_name, row.extra)
        loaded = self._config.models.embedder.get(target_embedder)
        if loaded is not None and stored == embedder_fingerprint(loaded.endpoint, loaded.model_name, loaded.extra):
            return
        logger.bind(embedder=target_embedder).info("Reloading model endpoints: the swap target was edited elsewhere")
        await refresh()
