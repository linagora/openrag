...
from core.models.partition import is_internal_partition
...
class RetrievalService:
    ...
    def _pipeline_groups_for_partitions(
        ...
        #
        # Every *user-facing* one, that is. An internal partition (a run's
        # throwaway ``__eval_*``) is hydrated like any other — it has to be,
        # because the run measures retrieval by searching it by name — but it
        # is filtered out of the partition listings, so letting the wildcard
        # put it back would feed an admin's ``openrag-all`` chat context from a
        # partition no listing shows and no detail view can reach. Named access
        # is unaffected; only the meaning of "all" is narrowed.
        if "all" in partitions and configs:
            partitions = [name for name in configs if not is_internal_partition(name)]
