...
2. **Replace the ``default`` alias with the embedder it resolves to today, on
   partitions that hold files.** A partition on the alias follows every
   set-default. With one shared field that silently mixed vector spaces; with a
   field per embedder it would point the partition at a field its rows were
   never copied into. The application pins such a partition on its first write
   and on set-default (#939), so only files indexed before that remain to pin
   here. Empty partitions keep the alias: following the default is what it is
   for, and they have no rows to route.
...
# Same test the application pins by (_PIN_INDEXED_ALIAS_PARTITIONS_SQL).
...
def _pin_default_alias(conn: sa.engine.Connection) -> None:
    ...
    if len(defaults) != 1:
        ...
        # choice a guess. Either way the indexed partitions riding it cannot be
        # routed to a dense field, so stop rather than migrate into that state —
        # the operator sets a default and runs this again. No indexed partition
        # on the alias, nothing to pin.
        riding = conn.execute(
        ...
        if not riding:
            return
        raise RuntimeError(
            f"{riding} partition(s) holding files use the '{_DEFAULT_ALIAS}' embedder alias, and this "
            f"deployment has {len(defaults)} default embedder endpoints. Each embedder now owns its own "
            "vector field (#762), so the alias has to be pinned to one concrete embedder before those "
            "partitions can be read or written. Mark exactly one embedder endpoint as the default, then "
