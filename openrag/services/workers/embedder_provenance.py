"""Per-file embedder provenance: what produced a file's vectors, and where they are.

Kept apart from :mod:`services.workers.pipeline_builder` so the API-side
callers that record it — the dispatcher's copy path and the embedder swap —
do not import the indexing pipeline, and with it every stage, the inference
runtime and the language detector.
"""
