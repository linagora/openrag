"""Pydantic Settings models replacing Hydra/OmegaConf configuration.

Each nested model corresponds to a section in the old .hydra_config/config.yaml.
Environment variables are mapped via `validation_alias` where the env var name
...
# ---------------------------------------------------------------------------
# Helper to read an env var with an optional default, coercing types.
# ---------------------------------------------------------------------------
...
class RerankerConfig(ConfigMixin):
    ...
    def from_env(cls) -> RerankerConfig:
        ...
        # Handle the OmegaConf interpolation:
        # base_url default was: http://reranker:${oc.env:RERANKER_PORT, 7997}
...
class ServerConfig(ConfigMixin):
    ...
    def from_env(cls) -> ServerConfig:
        ...
        # Hydra had "null" as a default string — treat "null" as None
...
class MimetypesConfig(ConfigMixin):
    ...
    # Field names use underscores because "/" is not valid in Python identifiers.
    # We store as a plain dict for iteration.
...
class LoaderConfig(ConfigMixin):
    ...
    # Docling settings (not in default config but referenced in code)
