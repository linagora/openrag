...
class RetrievalDiagnosticsGuard:
    ...
    def __init__(self, rate_limit: str | None = None) -> None:
        configured = rate_limit or os.getenv("RETRIEVAL_DIAGNOSTICS_RATE_LIMIT", _DEFAULT_RATE_LIMIT)
        try:
            self._limit = parse(configured)
        except ValueError:
            logger.bind(configured_value=configured).warning(
                "Invalid retrieval diagnostics rate limit; using the safe default"
            )
            self._limit = parse(_DEFAULT_RATE_LIMIT)
