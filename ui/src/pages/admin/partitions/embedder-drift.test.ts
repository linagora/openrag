...
describe("computeEmbedderDrift", () => {
  ...
  it("splits one model served at two widths, and flags both", () => {
    // Two vector spaces under one name: whichever width the partition reads
    // today, the files at the other width are not in that index at all.
    const drift = computeEmbedderDrift(
      "qwen",
      [
    ...
    expect(drift.hasDrift).toBe(true);
    expect(drift.driftedFiles).toBe(9);
  });
