...
describe("computeEmbedderDrift", () => {
  ...
  it("flags files sitting in another endpoint's field, same model or not", () => {
    // Two endpoints, one model, a field each: searches read the
  ...
  it("gives the same drift the same signature whatever order the rows arrive in", () => {
    // Equal counts keep the API's row order; a reordered response must not
    // bring back a warning already dismissed.
    const rows = [
      { embedder: "bge-m3", model_name: "bge-m3", dimension: 1024, file_count: 2 },
      { embedder: "e5", model_name: "e5-large", dimension: 1024, file_count: 2 },
    ];
