...
const OpenRagRuntimeProviderInner = ({
  ...
  // Wrap the base adapter so that each completed turn is persisted to the
  // store. Each turn appends exactly the latest user message plus the new
  // assistant message; prior history already lives in the store.
  const adapter = useMemo(
    () => createPersistingAdapter(baseAdapter, store, conversationId),
