...
10. [Phase 4 — ABCs & Ports](#phase-4--abcs--ports)
...
### Current codebase (openrag 1.1.9)
...
lives in the indexing service/pipeline. Models are serialized to dicts by caller,
...
|   |   |-- chunking.py, embedding.py, indexation.py,
|   |   |   infrastructure.py, loader.py, partition.py, presets.py,
|   |   |   retrieval.py, root.py
|   |   +-- __init__.py
|   |
|   |-- ports/                   # CRUD repository contracts (composed inside CatalogStore)
...
|   |
|   |-- catalog/                 # ABC (impls in services/storage/)
|   |   |-- __init__.py          # re-exports CatalogStore
|   |   +-- catalog_store.py     # CatalogStore ABC - composite repo aggregate root
|   |
...
`catalog/`, and `indexing/parsers/`.

Ports (`core/ports/`) are CRUD repositories composed inside CatalogStore.
...
```bash
mkdir -p openrag/core/{config,ports,models,chunking,embeddings,rerankers,llm,vlm,prompts,vector_stores,catalog,retrieval,indexing/parsers,observability,utils}
...
| `core/config/embedding.py` | `EmbedderConfig` + `ModelEndpointConfig` pattern |
...
| `core/catalog/catalog_store.py` | `CatalogStore` | `async initialize()`, `async shutdown()`, properties: `document_repo`, `job_repo`, `user_repo`, `prompt_repo`, `partition_repo`, … (13 repos) |
...
from openrag.core.catalog.catalog_store import CatalogStore
...
4.6  Create core/catalog/{catalog_store.py, __init__.py}
4.7  Create core/chunking/{chunking_strategy.py, registry.py, __init__.py}
4.8  Create core/indexing/parsers/{document_parser.py, registry.py, __init__.py}
4.9  Create all port ABCs in core/ports/
```
