...
| **Compose** | The Ray agent, unless the monitoring overlay (`monitoring.docker-compose.yaml`) is used: it pins `RAY_METRICS_EXPORT_PORT` so the `openrag-ray` scrape job can reach it. Ray otherwise picks a random port. | Without the overlay's pinned port, as the first row. |
