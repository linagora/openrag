...
##### Inference services: external endpoints, or run them locally

The stack does **not** start an embedder or reranker of its own — it expects them to be **external endpoints** you configure in `.env`, so you can reuse existing inference servers and keep the footprint small.

- **Embedder** — set `EMBEDDER_BASE_URL` and `EMBEDDER_API_KEY` to your embedding endpoint.
- **Reranker** — set `RERANKER_ENABLED=false` to skip it, or point it at an endpoint with `RERANKER_PROVIDER=openai` and `RERANKER_BASE_URL` (+ `RERANKER_API_KEY`).

With those endpoints configured, **no inference container is started** — running the core stack is all you need:
...
To run the embedder and/or reranker **locally** instead, uncomment the `vllm-gpu` (or `vllm-cpu`) service and the `extern/reranker/…` line in the `include:` block of `infra/compose/docker-compose.yaml`, then start the stack the same way.
