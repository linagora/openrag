...
Only allowlisted English and French casual messages skip retrieval; other phrasing retrieves by default. Casual
replies use a dedicated instruction in the detected language. Only greetings include an
introduction; set `ASSISTANT_NAME` to use your deployment's name. Ambiguous or normalized-empty input defaults to English.
