...
:::caution
On a partition that holds indexed files, changing `embedder` here returns `409 EMBEDDER_SWAP_REQUIRED`. Each
embedder stores its vectors in its own field and a partition searches only its own, so repointing the field
without re-embedding would hide every indexed file. Use the embedder swap below, which re-embeds first. The
edit is still accepted on a partition with no files.
:::

#### Partition Embedder Swap

Move a partition to another embedder, re-embedding its files in place. Each chunk's **stored text** is embedded
again with the new embedder — nothing is re-parsed or re-chunked — and searches keep using the current embedder
until every file is done, so no query returns results from a half-filled index.

| Endpoint | Method | Role | Description |
|----------|--------|------|-------------|
| `/partition/{partition}/embedder-swap` | POST | Owner | Start a swap → `202` with the swap |
| `/partition/{partition}/embedder-swap` | GET | Viewer | Progress, or how the last one ended → `404 EMBEDDER_SWAP_NOT_FOUND` if the partition never had one |
...
The response carries `status` (`running` | `completed` | `failed` | `cancelled`), `files_done` of `files_total`,
and `error` when it failed. There is no push notification: poll the `GET` route for progress. A swap onto the
embedder the partition already uses re-embeds only the files recorded with another model, which is how drift
from an earlier change is repaired.
...
`409 EMBEDDER_SWAP_IN_PROGRESS`; deleting files stays allowed. Starting one is refused with
`409 INDEXING_IN_PROGRESS` while files are still being indexed.

Cancelling leaves the partition on its current embedder. Files already re-embedded keep their new vectors —
nothing searches them until a later swap to that same embedder, which skips them.
:::
...
`DELETE` of an embedder returns `409` while a partition names it or, for the default embedder, while a partition following the `default` alias already holds indexed files. Reassign those partitions first.
...
For embedders, a new default only reaches partitions that have no data yet. A partition stays on the `default` alias until it first receives a file (upload or copy), at which point the alias is replaced by the embedder it resolved to; a partition indexed before that is pinned the same way when the default changes. Partitions with files therefore keep the embedder that built their vectors, and an admin moves one deliberately by setting its `embedder`. An emptied partition stays pinned — set its `embedder` back to `default` to follow the default again.
