...
`file_rev`/`datetime`/`doctype` are echoed verbatim from the upload metadata,
letting the receiver detect a stale status. Server-side keys (on-disk path,
content hash, file size) are never sent.
