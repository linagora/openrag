...
| `top_k` (optional)             | integer | 5       | Number of initial results, from 1 to 200 (optional) |
...
| `related_limit` (optional)     | integer | 20      | Max related/ancestor chunks to fetch per result, from 0 to 100 (used when `include_related` or `include_ancestors` is true) |
...
| `compare_original_query` | Chat only | `bool` | `false` | For administrators with tracing enabled, runs an isolated retrieval for the original message and stores it under `comparisons.original_query`. It never changes the answer's documents. |
