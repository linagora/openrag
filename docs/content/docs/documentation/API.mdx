...
| `chunk_contextualizer`, `image_captioning`, `topic_tagger`, `asr_transcription` | indexation preset — `*_prompt_name` |
...
Preset-scoped prompts are selected the same way, by putting the `*_prompt_name` field in the preset's `config` (see [Pipeline Presets](#-pipeline-presets)). The indexation preset also selects the STT endpoint used for its audio files.
