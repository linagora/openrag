...
OpenRAG supports document relationships to enable context-aware retrieval. You can model relationships between files using the `metadata` field during upload.

###### **For Folder-Related Documents**
To represent native folder-based relationships between files, rely exclusively on `relationship_id`:
...
**Key Points:**
- `relationship_id`: Represents the logical grouping, such as a project name or folder path (for example: `documents/projects/2024/q1`, `folder1`, `folder2`).

###### **For Email Threads**
To keep track of email threads and reply hierarchies, use both `relationship_id` (thread grouping) and `parent_id` (reply hierarchy):
...
**Key Points:**
- `relationship_id`: Email thread ID (same for all emails in the conversation)
- `parent_id`: ID of the email being replied to (null for root email)
- Email should be indexed as a separate file

**Benefits:**
- Context-aware search retrieval (see [search parameters](/openrag/documentation/API#search-across-multiple-partitions))
- Use `include_related=true` to get all files in the same folder/thread
- Use `include_ancestors=true` to get parent emails in the reply chain (emails only)

For more details, see the [Document Relationships documentation](/openrag/documentation/linked_files).
...
- `include_related` (optional): Include chunks from files with same relationship_id (default: false)
  * use for folder-related documents to get all files in the same folder/thread
- `include_ancestors` (optional): Include ancestor documents based on parent_id (default: false)
  * use for email threads to get parent emails in the reply chain
