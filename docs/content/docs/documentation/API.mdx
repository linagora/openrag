...
| `top_k` (optional)             | integer | 5       | Number of initial results, from 1 to 1000 (optional) |
...
| `related_limit` (optional)     | integer | 20      | Max related/ancestor chunks to fetch per result, starting at 0 (used when `include_related` or `include_ancestors` is true) |
...
| `include_retrieval_trace` (optional) | boolean | `false` | Adds schema-versioned, content-free retrieval diagnostics for administrators. May execute additional dense and sparse diagnostic searches. |
...
Tracing is administrator-only and opt-in because it exposes query and candidate identifiers, adds diagnostic work,
and can make responses large. The trace is returned as
`retrieval_trace` and contains identifiers, ranks, public scores, decisions, timings, and configuration identity;
it excludes document content, prompt content, credentials, and internal endpoint addresses. If a diagnostic leg fails, the
search result remains authoritative and the affected stage is marked unavailable or errored.

* Retrieval Snapshot
```http
GET /search/partition/{partition}/snapshot
```

Returns the allowlisted retrieval, model, and index configuration used to reproduce a run. This endpoint is
administrator-only. Set `include_document_ids=true` only for index-health checks; responses contain at most 10,000
identifiers and report whether the list was truncated. Document identifiers are excluded from the snapshot fingerprint
so compact and expanded snapshots remain comparable.
...
| `include_retrieval_trace` | Chat only | `bool` | `false` | For administrators, adds trace schema version 1 to `extra.retrieval_trace`. The streaming endpoint emits it only in terminal metadata. |
| `compare_original_query` | Chat only | `bool` | `false` | For administrators, runs an isolated retrieval for the original message and stores it under `comparisons.original_query`. Requires `include_retrieval_trace: true`; it never changes the answer's documents. |
| `bypass_query_contextualization` | Chat only | `bool` | `false` | Uses the latest user message as the exact retrieval query instead of rewriting it. JSON `true` requires both `include_retrieval_trace: true` and `require_retrieval: true`, and cannot be combined with `compare_original_query`. `false` is ignored. |
| `retrieval_similarity_threshold` | Chat only | `number` | `null` | Diagnostic per-request similarity threshold override. A non-null value must be finite and between `0` and `1`, inclusive. Active values require both `include_retrieval_trace: true` and `require_retrieval: true`, and cannot be combined with `websearch`. `null` is ignored. |
| `retrieval_top_k` | Chat only | `integer` | `null` | Diagnostic per-request retrieval result limit. A non-null value must be between `1` and `1000`, inclusive. Active values require both `include_retrieval_trace: true` and `require_retrieval: true`, and cannot be combined with `websearch`. `null` is ignored. |
| `retrieval_disable_reranker` | Chat only | `bool` | `false` | JSON `true` disables reranking for this diagnostic request. It requires both `include_retrieval_trace: true` and `require_retrieval: true`, and cannot be combined with `websearch`. `false` and `null` are ignored. |
| `retrieval_disable_expansion` | Chat only | `bool` | `false` | JSON `true` disables related-document and ancestor expansion for this diagnostic request. It requires both `include_retrieval_trace: true` and `require_retrieval: true`, and cannot be combined with `websearch`. `false` and `null` are ignored. |

Chat tracing persists the exact contextualized query, temporal filters, retrieval decision, fallback state, prompt
hash, and retrieval stages. It is not regenerated when results are inspected. Both tracing and original-query
comparison add work, so they should be reserved for evaluation and diagnostics.
