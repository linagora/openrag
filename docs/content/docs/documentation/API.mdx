...
| `include_retrieval_trace` (optional) | boolean | `false` | Adds schema-versioned, content-free retrieval diagnostics. May execute additional dense and sparse diagnostic searches. |
...
Tracing is opt-in because it adds diagnostic work and can make responses large. The trace is returned as
`retrieval_trace` and contains identifiers, ranks, public scores, decisions, timings, and configuration identity;
it excludes document content, prompt content, credentials, and endpoint secrets. If a diagnostic leg fails, the
...
Returns the allowlisted retrieval, model, and index configuration used to reproduce a run. Set
`include_document_ids=true` only for index-health checks; document identifiers are excluded from the snapshot
fingerprint so compact and expanded snapshots remain comparable.
...
| `include_retrieval_trace` | Chat only | `bool` | `false` | Adds trace schema version 1 to `extra.retrieval_trace`. The streaming endpoint emits it only in terminal metadata. |
| `compare_original_query` | Chat only | `bool` | `false` | With tracing enabled, runs an isolated retrieval for the original message and stores it under `comparisons.original_query`. It never changes the answer's documents. |
