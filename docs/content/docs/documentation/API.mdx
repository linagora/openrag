...
|--------|------|---------|-------------|
| `require_retrieval` | `bool` | `false` | Set to JSON `true` for evidence verification or retrieval evaluation. Partition-backed chat (including streaming) and text completions attempt retrieval even when the query router would skip it, using the original input as a fallback. Existing scope and filters still apply; matching sources and answer correctness are not guaranteed. Has no effect in direct LLM mode. |
