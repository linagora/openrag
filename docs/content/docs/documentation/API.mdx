...
| `include_retrieval_trace` (optional) | boolean | `false` | Add content-free retrieval diagnostics for administrators. This may execute additional search work. |
...
Tracing is opt-in and administrator-only because it exposes query and candidate identifiers and performs extra diagnostic
searches. It reports identifiers, ranks, public scores, decisions, timings, and configuration identity. Document content,
prompt content, credentials, and internal endpoint addresses are excluded.
...
Returns the public retrieval, model, and index identity needed to reproduce a benchmark run. The endpoint is
administrator-only. Document identifiers are optional, bounded, and excluded from the snapshot fingerprint so compact
and expanded snapshots remain comparable.

* Retrieval Diagnostics
```http
POST /search/partition/{partition}/diagnostics
```

Runs contextualization, retrieval, and reranking without generating an answer. This experimental administrator-only
endpoint is intended for reproducible evaluations that should not pay for an unused answer. It supports contextualized,
original, and paired comparison modes with bounded retrieval overrides. Responses contain only the sanitized trace.
...
| `include_retrieval_trace` | Chat only | `bool` | `false` | For administrators, adds trace schema version 1 to `extra.retrieval_trace`. Streaming responses emit it only in terminal metadata. |
| `compare_original_query` | Chat only | `bool` | `false` | Runs an isolated retrieval for the original message without changing the documents used for the answer. Requires tracing. |
| `bypass_query_contextualization` | Chat only | `bool` | `false` | Uses the latest user message as the exact retrieval query. Requires tracing and forced retrieval, and cannot be combined with original-query comparison. |
| `retrieval_similarity_threshold` | Chat only | `number` | `null` | Per-request diagnostic threshold from `0` to `1`. Requires tracing and forced retrieval. |
| `retrieval_top_k` | Chat only | `integer` | `null` | Per-request diagnostic retrieval depth from `1` to `1000`. Requires tracing and forced retrieval. |
| `retrieval_disable_reranker` | Chat only | `bool` | `false` | Disables reranking for this diagnostic request. Requires tracing and forced retrieval. |
| `retrieval_disable_expansion` | Chat only | `bool` | `false` | Disables related and ancestor expansion for this diagnostic request. Requires tracing and forced retrieval. |

Chat tracing persists the exact contextualized query, retrieval decision, fallback state, prompt identity, and retrieval
stages. It is never regenerated when a run is inspected. Tracing and original-query comparison add work and are intended
