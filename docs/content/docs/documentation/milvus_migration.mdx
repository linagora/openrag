...
While the Milvus 2.6 stack still exists, resolve the exact host directory used by the current container:
...
Stop the stack, take the backup described above, and then transfer ownership to the Milvus 3 user. Do not add `--volumes` to the shutdown command.
