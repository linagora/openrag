...
The message-queue type must also remain unchanged during the upgrade. OpenRAG's Milvus 2.6 standalone deployment uses RocksMQ by default, so add the following value to the Compose `.env` file before updating. If the deployment explicitly uses another queue, preserve that value instead. Do not combine the version upgrade with a queue migration; use the separate [Milvus queue-switch procedure](https://milvus.io/docs/switch-rocksmq-woodpecker.md) after version 3 is healthy.
...
Run these commands from `infra/compose` while the Milvus 2.6 stack still exists. First resolve the exact host directory used by the current container:

```bash
MILVUS_CONTAINER="$(docker compose ps -q milvus)"
MILVUS_DATA_PATH="$(
  docker inspect "$MILVUS_CONTAINER" \
    --format '{{range .Mounts}}{{if eq .Destination "/var/lib/milvus"}}{{.Source}}{{end}}{{end}}'
)"

test -n "$MILVUS_DATA_PATH"
test -d "$MILVUS_DATA_PATH"
printf 'Milvus data directory: %s\n' "$MILVUS_DATA_PATH"
```

Stop the stack, take the backup described above, and then transfer ownership to the Milvus 3 user:

```bash
docker compose down
sudo chown -R 999:999 "$MILVUS_DATA_PATH"
```
...
The default OpenRAG 2.5 standalone deployment uses RocksMQ. Keep `mq.type` set to `rocksmq` in every Compose configuration used for the 2.6.11 step, then retain the `MILVUS_MQ_TYPE` setting required by the 2.6-to-3.0 procedure above. If the deployment uses a custom queue, preserve that queue instead.

Before starting the intermediate OpenRAG release, add `MQ_TYPE: rocksmq` to the Milvus service's environment. When moving to the current release, keep the following value in the Compose `.env` file until the upgrade is validated:
