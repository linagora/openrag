...
Fresh installations need no manual ownership step. The Compose stack initializes the empty Milvus bind mount before the server starts. The procedure below remains mandatory for existing data because all files created by Milvus 2.6 must be transferred to the Milvus 3 user.
...
- If you run Milvus **2.5.x or earlier**, first complete the legacy intermediate upgrade to 2.6.11, then follow the 2.6-to-3.0 procedure.
:::
...
Milvus 2.6 containers ran as root, so existing bind-mounted files are commonly owned by `root:root`. Milvus 3 runs as UID/GID `999:999` and cannot start until it owns the directory mounted at `/var/lib/milvus`.
...
This intermediate path is required for deployments upgrading from **OpenRAG &lt;= 1.1.7**. Do not jump directly from Milvus 2.5.x to 3.0.0.
