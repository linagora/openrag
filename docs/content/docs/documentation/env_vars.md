...
| `MARKER_PARSE_MEMORY_LIMIT_MB` | int | 0 | Ceiling on what one Marker child may allocate (`RLIMIT_DATA`), so a single oversized parse raises `MemoryError` in that child instead of OOM-killing the pod. `0` disables it. Bounds the child's **whole** memory — torch and Marker's startup included — so it must sit well above the child's baseline; the worker logs that baseline at startup, and warns when the headroom is too small. With page chunking on, the bound applies per chunk. Measure per deployment before enabling. |
