...
| `METRICS_TOKEN` | `string` | unset | Bearer a Prometheus scraper must send on `GET /metrics`. Unset leaves the endpoint open to anyone reaching the API port (the route bypasses the auth middleware; admin tokens are not accepted there). Set it whenever the API is exposed through a public proxy. See [Prometheus metrics](/openrag/documentation/prometheus_metrics/). |
