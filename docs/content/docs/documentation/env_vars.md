...
The bundled transcriber image is pinned for Whisper and predates MOSS support. Serve MOSS with the vLLM build specified by the model authors, then use it as the external transcription endpoint. Its advanced STT request options are sent unchanged to vLLM, which allows an appropriate output-token budget and deterministic JSON output to be managed with the endpoint. For a MOSS model, **Admin UI → Model Endpoints** also offers an optional **Speaker-aware MOSS transcript** setting. When enabled, OpenRAG removes timecodes from complete diarized responses and keeps normalized speaker labels only when MOSS identifies more than one speaker. Leave it disabled to preserve the provider response unchanged. This post-processing also works with served-model aliases and is never sent to the provider. Manage the transcription instruction in **Admin UI → Prompt Library → Transcription**; changes apply to the next transcription without a restart, and an empty instruction retains MOSS's built-in behavior.
:::
