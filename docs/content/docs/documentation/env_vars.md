...
| `OPENRAG_INTERNAL_URL` | `string` | `http://openrag:8080` | Base URL the evaluation runner uses to reach the API from the Ray container (it uploads the corpus and drives promptfoo over HTTP). Change only if the API is not reachable under the compose service name. |
