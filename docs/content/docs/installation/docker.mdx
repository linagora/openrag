...
`GRAFANA_ADMIN_USER` defaults to `admin`. The bundled Prometheus scrapes
`GET /metrics` on the API over the Compose network without any token; see
[Prometheus metrics](/openrag/documentation/prometheus_metrics/) to protect
the endpoint with `METRICS_TOKEN` or to scrape it from an external Prometheus.
...
Set `LOKI_URL` (and the optional `LOKI_USERNAME`, `LOKI_PASSWORD`,
`LOKI_TENANT_ID`) in `.env`. See [Loki logs](/openrag/documentation/loki_logs/).
